...
- **Push-to-Talk**: Голосовой ввод с удержанием горячей клавиши (запись с микрофона → LLM)
...
- **Запись голоса**: Удерживайте `F9` (по умолчанию) для записи с микрофона
- **Обработка**: После отпускания клавиши аудио отправляется в LLM для транскрибации
- **Результат**: Текст автоматически вставляется в активное приложение

Голосовой ввод настраивается в файле `config.yaml` в разделе `ptt`:
- `enabled` - включение/выключение PTT (true/false)
- `hotkey` - горячая клавиша (F1-F12, PAUSE, SCROLL)
- `llm_path` - путь к LLM утилите (по умолчанию mistral.exe)
- `llm_args` - аргументы для LLM (по умолчанию "-m audio")
- `system_prompt` - системный промпт для транскрибации
...
- **Push-to-Talk** - включение/выключение голосового ввода
...
- (Опционально) GCC (MinGW) для компиляции Push-to-Talk функционала

### Установка GCC для Push-to-Talk

Для работы голосового ввода (PTT) требуется GCC. Установите MinGW-w64:

1. Скачайте с [GitHub Releases](https://github.com/niXman/mingw-builds-binaries/releases)
2. Распакуйте в `C:\mingw64`
3. Добавьте `C:\mingw64\bin` в системную переменную PATH
4. Перезапустите терминал и проверьте: `gcc --version`
